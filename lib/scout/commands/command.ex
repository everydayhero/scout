defmodule Scout.Commands.Command do
  ...
  * `validate: function`, expands to `Ecto.Changeset.validate_change/3`
  ...
  defp validation_visitor(node = {:validate, args}, validators) do
    validation_ast =
      quote do
        ...
        fn changeset, field ->
          Changeset.validate_change(changeset, field, custom_validation_func)
        end
      end
  ...
  defp validation_visitor(node = {:format, args}, validators) do
    validation_ast =
      quote do
        fn changeset, field -> Changeset.validate_format(changeset, field, unquote(args)) end
      end
  ...
  defp validation_visitor(node = {:length, args}, validators) do
    validation_ast =
      quote do
        fn changeset, field -> Changeset.validate_length(changeset, field, unquote(args)) end
      end
  ...
  defp validation_visitor(node = {:required, true}, validators) do
    validation_ast =
      quote do
        fn changeset, field -> Changeset.validate_required(changeset, field) end
      end
  ...
  defp attribute_visitor(
         ...
         validation_asts: validation_funcs
       ) do
    validation_func_ast =
      quote do
        fn changeset ->
  ...
  defp attribute_visitor(
       ...
       ) do
    {
      ...
      [
        field_names: field_names,
        validation_asts: [changeset_validation_func_ast | validation_funcs]
      ]
    }
  end
  ...
  defp attribute_visitor(
    ...
    validation_func_ast =
      quote do
        fn changeset ->
          Enum.reduce(unquote(validators_for_field), changeset, fn validator, changeset ->
...

    {
      ...
      [
        ...
        validation_asts: [validation_func_ast | validation_funcs]
      ]
    }
  end
  ...
  defp build_type_ast(fields_ast = {:__block__, _, _}, module) do
    Macro.postwalk(fields_ast, fn
      {:field, _, [name, type | _rest]} ->
        quote do
          {unquote(name), unquote(ecto_to_typespec(type))}
        end

      {:embeds_one, _, [name, type | _rest]} ->
        quote do
          {unquote(name), unquote(type).t}
        end

      {:embeds_many, _, [name, type | _rest]} ->
        quote do
          {unquote(name), [unquote(type).t]}
        end

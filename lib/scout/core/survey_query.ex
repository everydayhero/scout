defmodule Scout.SurveyQuery do
  ...
  def build(params) do
    Enum.reduce_while(params, Scout.Survey, fn
      {"owner", id}, query ->
        {:cont, query |> for_owner(id)}

      {"name", pattern}, query ->
        {:cont, query |> name_like(pattern)}

      {"state", state}, query ->
        {:cont, query |> in_state(state)}

      {"started", "+" <> t}, query ->
        {:cont, query |> started_after(t |> DateTime.from_iso8601() |> elem(1))}

      {"finished", "-" <> t}, query ->
        {:cont, query |> finished_before(t |> DateTime.from_iso8601() |> elem(1))}

      {other, _}, _query ->
        {:halt, {:error, %{other => "invalid parameter"}}}
    end)
  end
